...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if ('IntersectionObserver' in window) {
    ...
    document.querySelectorAll(
      '.stat-card, .service-card, .test-category-card, ' +
      '.package-card, .doctor-card, .gallery-item, ' +
      '.contact-item, .about-grid, .section-header'
    ).forEach(function (el) {
  ...
  var lastCartCount = 0;
  var cartCountObserver = new MutationObserver(function () {
    var newCount = parseInt(cartCountEl.textContent, 10) || 0;
  ...
  cartCountObserver.observe(cartCountEl, { childList: true, characterData: true, subtree: true });
